...
int main()
{
    ...
    while (1)
    {
        ...
        else if (numOfTokens == 1 && strcmp(tokens[0], "quit") == 0)
        {
            ...
            int status = setenv(tokens[1], tokens[2], 1);
            // printf("set %s as %s stat %d\n", tokens[1], tokens[2], status);
            // printf("%s\n", getenv(tokens[1]));
        } else if (strcmp(tokens[0], "unset") == 0) {
            int status = unsetenv(tokens[1]+1);
            // printf("unset %s stat %d\n", tokens[1]+1, status);
            // printf("%s\n", getenv(tokens[1]+1));
        }
        else {
            for(int i=0;i<numOfTokens;i++) {
                if(tokens[i][0]=='$') {
                    char* temp = getenv(tokens[i]+1);   
                    if(temp!=NULL) strcpy(tokens[i],temp);
                    else strcpy(tokens[i], "");
                }
            }
            int totalJobs;
            int *tokenCountByJob;
